...
def main(args):
    ...
    if args.list:
        downloadable_ids = []
        for k, v in assrc.DATA_SOURCES.items():
    ...
    if args.uid == "":
        ...
        print(list(assrc.DATA_GROUPS.keys()))
        return
    if args.uid == "all":
        if verbose:
            print("All assets will be downloaded. This may take a while.")
        uids = list(assrc.DATA_SOURCES.keys())
        show_progress = True
    elif args.uid in assrc.DATA_GROUPS:
        uids = assrc.expand_data_group_into_individual_data_source_ids(args.uid)
        show_progress = True
    elif args.uid in assrc.DATA_SOURCES:
        uids = [args.uid]
        show_progress = False
    else:
    ...
    for i, uid in enumerate(uids):
        ...
        output_path = download(assrc.DATA_SOURCES[uid], **kwargs)
        import time
        time.sleep(1)
