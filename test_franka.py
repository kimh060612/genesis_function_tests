...
franka_2 = scene.add_entity(
    gs.morphs.MJCF(
        ...
        pos=(0.0, 1.0, 0.0),
        quat=(0.7071067811865476, 0.0, 0.0, 0.7071067811865475) # (0, 0.0, 0.0, 1.0) (0.7071067811865476, 0.0, 0.0, 0.7071067811865475)
    ),
)
########################## build ##########################
scene.build()
...
for wp_1, wp_2 in zip(path, path_2):
    ...
    franka1_action.append(franka.get_dofs_force(motors_dof))
    franka2_action.append(franka_2.get_dofs_force(motors_dof_2))
...
qpos_2 = franka_2.inverse_kinematics(
    link=end_effector_2,
    pos=np.array([0.65, 0.52, 0.58]),
    quat=np.array([0.5, 0.52, 0.5, -0.5]),
)
print(qpos_2)
franka_2.control_dofs_position(qpos_2[:-2], motors_dof_2)
for i in range(200):
